  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [appsRes, plansRes, promosRes] = await Promise.all([
        ...
        supabase
          .from('pricing_plans')
          .select('id, app_id, name, price_monthly, price_yearly, features, is_active, max_users, max_locations, max_storage_gb, ai_monthly_parse_limit, has_ai_features, extra_user_price_monthly, extra_user_price_yearly, created_at, updated_at, applications(name)')
          .order('name'),
